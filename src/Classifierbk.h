...
public:
  ...
  std::vector<float> Predict(const vector<cv::Mat>& imgs, const vector<float>& joint, int clip_length_);
...
Classifier::Classifier(const string& model_file,
                       const string& trained_file):clip_length_(10)
{                                                                          
  //const string& label_file) {                                           
  /*#ifdef CPU_ONLY
...
std::vector<float> Classifier::Predict(const vector<cv::Mat>& imgs, const vector<float>& joint, int clip_length_) {
  Blob<float>* input_layer = net_->input_blobs()[0];
  ...
  Preprocess(imgs, input_channels, joint, joint_input, clip_input);
